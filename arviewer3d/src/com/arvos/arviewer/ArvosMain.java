...
import com.arvos.arviewer.viewer.ArvosAugment;
import com.arvos.arviewer.viewer.ArvosPoi;
import com.arvos.arviewer.viewer.ArvosPoiObject;
import com.arvos.arviewer.viewer.ArvosViewer;
...
public class ArvosMain extends ListActivity implements IArvosLocationReceiver, IArvosHttpReceiver
{
	...
	public void onHttpResponse(String url, String error, String text, Bitmap bitmap)
	{
		...
		boolean doRequestTextures = false;
		for (int i = 0; i < mAugments.size(); i++)
		{
			...
			if (url.equals(augment.mUrl))
			{
				...
				if (parsedAugment.mName == null)
				{
					parsedAugment.mName = name.trim();
				}
				mInstance.mAugment = parsedAugment;
				doRequestTextures = true;
				break;
			}
		}

		if (doRequestTextures)
		{
			requestTextures();
			return;
		}
	...
	{
		// Intent intent = new Intent(this, GamePlayNativeActivity.class);
